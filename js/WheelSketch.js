function WheelSketch(_p5) {
    ...
    _p5.setup = () => {
        ...
        const button = _p5.createButton('Крутить');
        button.parent(document.querySelector('.wheel-container'));
    ...
    function getTotalRowsForDurationAndSpeed(videoDurationSec = 5, speedItemsPerSec = 3) {
    ...
    function data_key(data_len, key) {
        if (!data_len) return key;
        if (key >= 0 && key < data_len) return key;
        if (key > 0) return data_key(data_len, Math.abs(data_len - key));
    ...
    function alignToRow(endCallback) {
        ...
        if (rest > half) newValue = counter + height_str - rest;
        animate(
            tickCounter,
            counter,
            newValue,
    ...
    function tickCounter(v) {
        ...
        if (!counterPrevTickValue) counterPrevTickValue = counter;
